...
export const useRedeemSecurity = (
  options: UseMutationOptions<boolean, unknown, RedeemRequest, unknown> = {},
) => {
  ...
  //@ts-ignore
  return useMutation(
    (redeemRequest: RedeemRequest) => SDKService.redeem(redeemRequest),
    {
      onSuccess: (data) => {
        if (data === true) {
          toast.show({
